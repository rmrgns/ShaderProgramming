...
const vec2 c_2DGravity = vec2(0.0, -5.0);
const float c_PI = 3.141592;

void Velocity()
{
	//float newTime = 2.0f*(fract(u_Time/u_Period));
...
void Line()
{
	...
	float newTime = abs(fract(u_Time/u_Period) - 0.5f) * 2.0;
	vec4 newPosition;
	newPosition.xyz = (a_Position+c_StartPos) + a_Velocity*newTime;
	newPosition.w = 1;
	gl_Position = newPosition;
}
...
void Circle()
{
	float newTime = -fract(u_Time / u_Period) * c_PI * 2.0f;
